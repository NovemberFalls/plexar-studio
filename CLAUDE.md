...
Cockpit surfaces a **machine-global** (not per-session) view of one or more local inference providers (e.g. LM Studio, Ollama, vLLM via a broker gateway). Providers are registered server-side; the browser never learns their base URLs (SSRF stance). Each provider declares its capabilities (queue, metrics, models, traces, health) and scope (`local` | `remote`).
...
- **Legacy routes:** Existing `/api/local/queue`, `/api/local/metrics` delegate to the default provider (thin wrappers).
...
**Removing it changed nothing a user could observe except the vanished controls.** Spill was inert three ways — shadow is the shipped default so `_queued_forward` never ran; nothing in Studio ever consumed the 503 (`lmstudio_proxy.py` has no spill branch); and there was no escalation target at request time. **Do not read this removal as a fix. Nobody ever experienced spill working.**

Two consequences, both deliberate and both stated where they live:

- **A request that would have been refused now WAITS.** It queues at its class priority and is served in turn. **The queue has no depth limit and no wait ceiling** — the client's timeout is the only backpressure left. `broker.py::_queued_forward`'s docstring says so, and `test_a_formerly_spillable_request_QUEUES_AND_COMPLETES` gates it on a declared total plus a depth ceiling rather than on "no 503 appeared".
- **Three surfaces lost a state rather than gaining a fake one.** The lane strip's fill bar, the Engine rail's `pressure` dot and Engine ▸ Live's "Interactive pressure" meter all measured *distance to the spill trigger*. With no trigger there is no fraction, so they are removed rather than re-based on an invented threshold (R38 / DEC-36).

`~/.plexar-studio/lane-broker/spills.jsonl` is **left on disk untouched**, the same stance as Chat's data: removing a feature must not remove the record. Nothing reads it; nothing deletes it.

### Managed lane broker (Cockpit owns the broker)

The broker is **vendored** at `web/lane_broker/` (pure stdlib, from the broker-team repo — keep byte-close to upstream; sync via the broker team). At startup, `start_managed_broker()` runs it **in-process as an asyncio task** on Cockpit's own loop (no subprocess → works inside the PyInstaller sidecar; pinned in `cockpit-server.spec` `hiddenimports` because the import is lazy). **Double-bind guard:** if anything already answers at the broker URL, external wins and Cockpit only proxies. Env knobs: `COCKPIT_MANAGED_BROKER` (default `1`), `COCKPIT_BROKER_SHADOW` (default `1` = observe+log only), `COCKPIT_LMSTUDIO_URL` (upstream, default `:1234`). State lives at `app_paths.data_path("lane-broker")/jobs.jsonl` — i.e. **`~/.plexar-studio/`** since the R-E split of 2026-08-02 (survives reinstall). This line said `~/.claude-cockpit/` until that morning and `~/.plexar/` until that evening; **`~/.plexar/` is the RIG's directory and is no longer ours.** Never hard-code any of them — call `app_paths.data_path()`, which is the single resolver and the only thing that knows.

**SHADOW IS THE DEFAULT, AND IT MEANS THE QUEUE NEVER RUNS** (S3/S10, measured in `lane_broker/tests/test_shadow_default_is_inert.py` against a real broker subprocess). In shadow the broker forwards and logs but skips `_queued_forward` entirely, so nothing is ever queued: `/queue` depth stays 0 forever. **Shadow is a BROKER concept, not a spill one** — it governs whether the queue runs at all, and it survives spill's removal unchanged.

- **This does NOT make the broker removable.** `lmstudio_proxy.py` posts every LM Studio session's `/v1/messages` at the broker's own address, so it is the TRANSPORT. Delete it and every session 503s. **Inert queueing is not an inert component.**
- **The UI must say so.** `broker.py` puts `shadow` in the `/queue` payload and `server.py` passes the body through, so the flag is on the wire — read it. `LaneStrip` renders a `queueing off · shadow` note instead of a live meter, and Settings ▸ Providers ▸ **Queueing** states which mode is live and why it cannot be switched here (`--shadow` is start-time only). That card is where SpillPolicy's shadow Callout went, and it is also what keeps Lane broker paired after spill's block was deleted. Do NOT infer shadow from a depth of zero: a healthy idle queueing broker reports 0/0 too, and an empty meter with no note claims an idle lane. Equally, do not simply HIDE the surfaces — absence reads as "this build has no queueing", which is a different claim from "queueing exists and is switched off" (the same collapse S9 removed from the model picker). `GET /api/local/status` carries `managed: bool`; the Connection card shows "managed by Cockpit" vs "external process". Broker failure never blocks Cockpit startup; shutdown cancels the task.
...
- The `vllm-local` provider declares capabilities `["models","health"]` ONLY — vLLM does not serve the broker's queue/traces shapes; a vLLM /metrics adapter is a future follow-up.
...
- **Polling (`App.jsx`):** Controlled by the `localEnabled` gate. Queue 3s; metrics/traces 10s (only when the capability exists). Best-effort errors silently render offline state.
...
`LocalBrokerView.jsx`, `LaneQueuePanel.jsx` and `LocalMetricsPanel.jsx` were deleted in the facelift. Their behavior was re-homed: spill thresholds → `settings/SpillPolicy.jsx` (itself removed 2026-08-03 with the feature), live queue → `engine/EngineRequests.jsx` + `engine/EngineLive.jsx` (now), reporting → `components/reports/` (the past), models folder → `settings/ProvidersSettings.jsx`.
