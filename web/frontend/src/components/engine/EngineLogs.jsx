 ...
 *
 * THERE IS NO LOG ENDPOINT TODAY. Plexar Studio's server logs through the `cockpit.*`
...
const WHERE_TODAY = [
  ...
  {
    what: "Lane broker",
    where: "the same stdout — the managed broker runs in-process on Plexar Studio's event loop, so its lines are interleaved with Plexar Studio's.",
  },
  {
    what: "vLLM",
      ...
      >
        ...
        >
          ...
          <p style={{ fontSize: 11, lineHeight: 1.6, color: "var(--cc-muted)", margin: 0 }}>
            Plexar Studio exposes no log endpoint, so there is nothing to stream into this tab. When one
