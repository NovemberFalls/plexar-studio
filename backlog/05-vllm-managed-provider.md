...
- ~~Level-2 "Cockpit as source of truth": derive the orchestration lane from Cockpit's connected provider instead of a hand-edited `local-lanes.json`.~~ **WITHDRAWN 2026-07-30 — wrong direction.** `local-lanes.json` is the rig's source of truth and stays that way; Cockpit *reads* the rig and does not own it, which is exactly why its vLLM provider is external by default. Making Cockpit authoritative would recreate the second-source-of-truth problem the Settings work spent this session removing (see the `claude_cli.binary_path` field that saved and changed nothing, and the `providers.vllm.managed` toggle nothing read).

  **Narrower replacement — surface declared-vs-served drift as a first-class state.** Cockpit already renders "on disk, not served" for models the engine cannot switch to; the same treatment should cover a served model or context window that disagrees with what the rig *declares*. Concrete: on 2026-07-30 `gpu-main` served `qwen3-30b-instruct` at `max_model_len` 32768 while three independent sources declared `qwen3-coder-30b-awq` at 49152 (`local-lanes.json`, `COCKPIT_VLLM_MAX_MODEL_LEN`, `local-rig/README.md`). Nothing detected it; a human noticed a `not loaded` badge.

  Read the declared side from `team/bench/local_lane/check_rig_drift.py --json`, whose row shape is the contract (`declared_model`/`declared_window`/`served_model`/`served_window`/`served[]`/`status ∈ {ok,DRIFT,absent,down}`/`siblings[]`; `served_*` is `null` when unknown, including on `down` and `absent`). One owner for the config, discrepancy shown where a human will see it.

  **Blast radius, correctly scoped:** Cockpit's context ring reads the window LIVE, so its reporting was never wrong. The *dispatcher* has no such protection — `local_worker.sh` sizes cards from the declared `session_ctx_budget`, so local routing since the flip was building cards against a window 16k short of reality. The damage sits exactly where nobody was looking, which is the argument for the indicator.
